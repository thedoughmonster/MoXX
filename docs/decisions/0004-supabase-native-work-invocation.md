...
A decision worker may call the exact versioned MoMi Order API route when its
durable work explicitly names that API contract. This is the only approved
Edge Function-to-Edge Function call.
...
owned Order API instead of allowing decision code to query warehouse tables.
