...
Repository architecture validation scans `execution-authorities/*.json` when that directory exists. An absent directory means no grants, not implicit authority. Validation indexes declarations once and never infers permission by repeatedly scanning runtime state.
