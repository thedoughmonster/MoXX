...
Repository architecture validation scans `execution-authorities/*.json` when that directory exists. An absent directory means no grants, not implicit authority. The repository scan validates each declaration's self-bound base and source identity; an execution consumer supplies the current base and accepted source digest and therefore detects drift. Accepted external evidence uses `external-authority:<key>:<operation>:<resource>` as its exact source identity. Validation indexes declarations once and never infers permission by repeatedly scanning runtime state.
