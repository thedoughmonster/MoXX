...
with base as (
  select work.id as delivery_work_id, work.idempotency_key,
    candidate.id as candidate_id, candidate.alert_kind,
    candidate.destination_key, candidate.order_presentation as presentation,
  ...
  from item_lines group by delivery_work_id
), prepared as (
  select base.*,
    case when alert_kind = 'preorder' then 'PREORDER' else 'ORDER' end
      as ticket_kind,
    case when alert_kind = 'preorder' then 'PICKUP' else 'READY' end
      as timing_label,
    replace(replace(replace(replace(coalesce(nullif(
    ...
    case when presentation ->> 'fulfillment_at' is null then 'Not provided'
      else to_char((presentation ->> 'fulfillment_at')::timestamptz
        at time zone 'America/New_York', 'Mon FMDD at FMHH12:MI AM')
      end as fulfillment_text,
    coalesce(item_text.item_text,
      'No readable line items were provided.') as item_text
  from base left join item_text using (delivery_work_id)
), tickets as (
  select prepared.*, format(E'%s #%s\nCUSTOMER: %s\n%s | %s %s\n'
      || E'%s %s | TOTAL %s\n\n%s', ticket_kind, display_number,
      customer_label, upper(source_label), timing_label, fulfillment_text,
      item_count, case when item_count::numeric = 1
        then 'ITEM' else 'ITEMS' end, total_text, item_text) as ticket_text
  from prepared
)
select delivery_work_id, candidate_id, destination_key,
  ...
  jsonb_build_object(
    ...
    'text', left(ticket_text, 4000), 'mrkdwn', false,
