  ...
  if source_event.source_reference ->> 'table' = 'webhook_events' then
    ...
    if webhook.subscription_key = 'orders' then
      return warehouse_projection.project_toast_order(
        'webhook:' || webhook.event_guid,
        'toast:event:' || p_event_id,
    ...
    selected_operation_key := case webhook.subscription_key
      when 'menus' then 'toast.menus.full.v1'
    ...
    select selected_operation_key, restaurant.source_key, webhook.restaurant_guid,
      'live', 'Webhook reconciliation: ' || webhook.subscription_key,
      source_event.correlation_id,
      'toast:webhook-reconcile:' || webhook.event_guid || ':' || selected_operation_key
    ...
    projection_result := case when found
