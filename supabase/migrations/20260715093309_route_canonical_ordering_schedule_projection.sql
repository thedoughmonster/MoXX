...
declare
  ...
  entity_type text; entity_id uuid; location_id uuid;
  canonical_document jsonb; provenance jsonb; version_id uuid;
...
begin
  ...
  if source_version.resource_type = 'ordering_schedule' then
    ...
    canonical_document := jsonb_strip_nulls(jsonb_build_object(
      'id', entity_id, 'entity_type', entity_type, 'location_id', location_id,
  ...
  provenance := jsonb_strip_nulls(jsonb_build_object(
    ...
    'projection_contract', case when source_version.resource_type =
      'ordering_schedule' then 'canonical-ordering-schedule-v1' end,
  ...
  insert into momi_events.events (
    ...
    observation.observed_at, 1, source_version.source_system,
    source_version.resource_type, source_version.source_id,
    jsonb_build_object('schema', 'momi_warehouse', 'table', 'entity_versions',
      'id', version_id), p_correlation_id
