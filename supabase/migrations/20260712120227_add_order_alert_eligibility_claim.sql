alter table toast_alerting.toast_sources
  add column order_guid_path text[] not null;
alter table toast_alerting.toast_sources
  add constraint toast_sources_order_guid_path_is_present
  check (array_length(order_guid_path, 1) > 0);
create unique index alert_rules_one_enabled_version_uidx
  on toast_alerting.alert_rules (source_key, alert_kind)
  where is_enabled;
create index alert_routes_destination_key_idx
  on toast_alerting.alert_routes (destination_key);
create index order_alert_candidates_destination_key_idx
  on toast_alerting.order_alert_candidates (destination_key);
create index order_alert_candidates_raw_event_id_idx
  on toast_alerting.order_alert_candidates (raw_event_id);
create index order_alert_candidates_route_idx
  on toast_alerting.order_alert_candidates (source_key, alert_kind);
create index order_alert_candidates_rule_id_idx
  on toast_alerting.order_alert_candidates (rule_id);
create function toast_alerting.claim_order_alert_candidates(
...
as $$
  ...
  matches as (
    ...
    where event.payload #> source.payload_path = source.expected_value
