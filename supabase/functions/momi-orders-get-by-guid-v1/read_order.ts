...
export async function readOrder(input: OrderReadInput): Promise<OrderLookup> {
  const rows = await sql<OrderReadRow[]>`
    with authorized_work as (
      select
        ...
        work.order_version_id::text as work_order_version_id,
        work.restaurant_guid
      from toast_hydration.order_api_invocation_work as work
      where work.id = ${input.work_id}::bigint
        and work.order_guid = ${input.order_guid}
    ...
    select
      work.work_id,
      work.work_order_version_id,
      coalesce(contract.contract_active, false) as contract_active,
      order_record.order_guid,
      order_record.restaurant_guid,
    ...
    left join lateral (
      select order_view.*
      from momi_api.toast_orders_by_guid_v1 as order_view
      where work.work_id is not null
        and contract.contract_active
        and order_view.order_guid = ${input.order_guid}
        and order_view.restaurant_guid = work.restaurant_guid
        and order_view.order_version_id = work.work_order_version_id::bigint
      limit 1
    ) as order_record on true
  `

  const row = rows[0]
  if (!row || row.work_id === null || row.work_order_version_id === null) {
  ...
  if (
    row.order_guid === null || row.restaurant_guid === null ||
    row.order_version_id === null || row.retrieved_at === null ||
