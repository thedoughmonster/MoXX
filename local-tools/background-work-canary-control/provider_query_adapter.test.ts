...
test("provider adapter uses exact argv, private temp files, and sanitized environment", async () => {
  ...
  try {
    const result = await executeProviderQuery({
      repositoryRoot: root, pnpmExecutable: process.execPath, sql,
      parser: (stdout) => new TextDecoder().decode(stdout),
    }, {
      temporaryRoot,
      environment: { PATH: "/usr/bin:/bin", SUPABASE_ACCESS_TOKEN: "private-token" },
      runChild: async (request) => {
        assert.equal(request.executable, process.execPath)
        assert.deepEqual(request.arguments.slice(0, 6),
          ["exec", "supabase", "db", "query", "--linked", "--file"])
        const sqlPath = request.arguments[6]
        assert.deepEqual(request.arguments.slice(7),
        ...
        assert.deepEqual(request.environment, {
          PATH: "/usr/bin:/bin", SUPABASE_TELEMETRY_DISABLED: "1",
        })
  ...
  try {
    for (const status of [
      ...
      const result = await executeProviderQuery({
        repositoryRoot: root, pnpmExecutable: process.execPath, sql,
        parser: () => "never",
      }, { temporaryRoot, environment: {}, runChild: async () =>
    ...
    }, { temporaryRoot, environment: {}, runChild: async () => childResult("success") })
...
test("provider adapter rejects unregistered SQL before spawning a child", async () => {
  ...
  try {
    const result = await executeProviderQuery({
      repositoryRoot: root, pnpmExecutable: process.execPath,
