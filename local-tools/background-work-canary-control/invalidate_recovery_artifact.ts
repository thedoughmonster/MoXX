...
export async function invalidateRecoveryArtifact(
  published: { path: string; sha256: string },
): Promise<void> {
  const directory = dirname(published.path)
  if (published.path !== join(directory, "recovery-final.json")) {
    throw new Error("Recovery artifact path is invalid")
  }
  ...
  if (sha256Text(bytes) !== published.sha256) {
    throw new Error("Recovery artifact changed before invalidation")
  }
  const invalidated = join(directory, "recovery-final.invalidated.json")
  ...
  if (persisted !== bytes || sha256Text(persisted) !== published.sha256) {
    throw new Error("Recovery artifact invalidation failed")
  }
}
