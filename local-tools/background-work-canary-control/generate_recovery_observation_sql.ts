...
export function generateRecoveryObservationSql(
  ...
  return transition.slice(0, boundary) + [
    ...
    "      or (select count(*) from toast_acquisition.schedules a",
    "        join toast_acquisition.operations o using (operation_key)",
    "        join toast_acquisition.sources x using (source_key)",
    "        join toast_acquisition.restaurants r using (source_key, restaurant_guid)",
    "        where a.active and o.is_enabled and x.is_enabled and r.is_enabled",
    "          and j.idempotency_key like a.schedule_key || ':%'",
    ...
    "            to_char(to_timestamp((sample->>'observedAtUtcMs')::bigint / 1000.0)",
    "              at time zone 'UTC', 'YYYYMMDDHH24MISS')",
    "          and (a.operation_key, a.source_key, a.restaurant_guid, a.mode) =",
    "            (j.operation_key, j.source_key, j.restaurant_guid, j.mode)) <> 1))",
