...
const groups: Array<[string, string, string[]]> = [
  ...
  ["missing_owner_contract", "proposal:event_exact_delivery_lifecycle", ["sha256:51196ed750600fa97fbf6fbcd3f27c32d22da9f325a8c9da1c7c7b7298ddf8a9", "sha256:496e0e7dc9c46fe33eca0e88033c943c230e3d4b4dcc86ab1a6b1439215027d0", "sha256:5d5e07c0ed09d1b3d9b0dd3464967020b86bebcea232e15e402923c10978f483", "sha256:061ab87a470ca6441c01ecd8e74de782fa1c3797169f14497fd03f447163834b"]],
...
const checks = {
  ...
  assertInventory(candidate: Inventory): void {
    ...
    assert.deepEqual([candidate.document_status, candidate.issue, candidate.base_sha,
      ...
      "docs/service-access-debt-baseline.json", "Implemented 25 of the original 35 private-access removals; the remaining 10 are intentionally deferred to separately owned follow-up work. Shared Edge credentials remain a known hardening dependency and do not prove per-service workload isolation."])
    assert.deepEqual([candidate.implemented_fingerprints,
      candidate.remaining_fingerprints], [25, 10])
    assert.equal(rows.length, 10)
    assert.equal(new Set(rows.map((row) => row.fingerprint)).size, 10)
    assert.deepEqual(groups.map((group) => group[2].length), [1, 4, 2, 3])
    ...
    assert.deepEqual(candidate.summary, { fingerprints: 10, virtual_subjects:
...
test("regenerates exact evidence and validates active origins", () => {
  ...
  assert.equal(baseline.findings.length, 97)
})
