...
test("passes a database credential only through the child environment", () => {
  const launcher = fileURLToPath(
    new URL("./fixtures/supabase_child_probe.ts", import.meta.url),
  )
  const output = runSupabase(["probe-argument"], true, "child-secret", launcher)
  ...
  assert.equal(child.databasePassword, "child-secret")
  assert.doesNotMatch(JSON.stringify(child.args), /child-secret/)
})
