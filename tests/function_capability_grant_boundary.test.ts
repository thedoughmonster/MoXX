...
function grantFor(service = "preorder-operations",
  calls: ExecutionAuthority["contracts"]["call"] = [{
...
test("admits only mapped database namespaces and the exact called contract", () => {
  assert.deepEqual(validate({ function_key: "momi.preorder.payment.initiate.v1",
...
test("rejects Square and OpenAI provider effects as caller authority", () => {
  ...
  const diagnostics = validate({ function_key: "momi.preorder.payment.initiate.v1",
...
test("admits reconciliation contract but rejects its provider authority", () => {
  const grant = grantFor("preorder-operations", [{
    provider_service: "square-payment-acquisition",
    contract: "square.payment.retrieve.v1",
  }])
  assert.deepEqual(validate({ function_key: "momi.preorder.payment.reconcile.v1",
    execution_authority: grant }), [])
  ...
  grant.external.invoke.push({ authority_key: "provider.square",
    operation: "retrieve", resource: "payment" })
  const diagnostics = validate({ function_key: "momi.preorder.payment.reconcile.v1",
    execution_authority: grant })
  assert.equal(diagnostics.filter((item) =>
...
test("admits webhook contracts but rejects provider and archive authority", () => {
  ...
  assert.deepEqual(validate({ function_key: "momi.preorder.square_webhook.process.v1",
    execution_authority: grant }), [])
  ...
  const diagnostics = validate({ function_key: "momi.preorder.square_webhook.process.v1",
    execution_authority: grant })
  assert.equal(diagnostics.filter((item) =>
