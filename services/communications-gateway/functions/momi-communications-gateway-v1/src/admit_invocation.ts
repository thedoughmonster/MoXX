...
export async function admitInvocation(
  ...
  const inputTokens = Math.ceil(JSON.stringify(input.messages).length / 4)
  const rows = await sql<Admission[]>`
    select disposition, invocation_id::text, provider_key, provider_model,
      provider_endpoint, maximum_output_tokens, timeout_seconds,
      maximum_attempt_cost_micros::text
