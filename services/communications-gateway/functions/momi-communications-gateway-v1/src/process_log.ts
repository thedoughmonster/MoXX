...
export async function processLog(input: ChatInput, selection: LogSelection): Promise<{
  ...
  const requestHash = await hashRequest(input, "momi.communications.log-intent.v1")
  const admitted = await admitLogInvocation(input, requestHash)
  if (admitted.disposition === "duplicate") return logReplayResponse(
    admitted.invocation_id,
    await loadInvocationReplay(input, admitted.invocation_id, requestHash),
  )
  ...
  try {
    ...
    if (!appended) throw new Error("curated_log_append_failed")
