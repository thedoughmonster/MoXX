...
import { pendingResponse } from "./pending_response.ts"
...
import { stageAsyncRound } from "./stage_async_round.ts"
...
const safeProviderErrors = new Set(["provider_background_deadline_exceeded",
  "provider_background_id_missing", "provider_background_poll_failed",
...
export async function executeAdmittedChat(input: ChatInput, admission: Admission,
  ...
  while (true) {
    ...
    if (route.route_key === "maximum" &&
      (initial.body.status === "queued" || initial.body.status === "in_progress")) {
      const responseId = providerResponseId(initial.body)
      if (!initial.gateway_call_id || !responseId) {
        throw new Error("provider_background_id_missing")
      }
      await captureEvidence(input, admission.invocation_id,
        ++evidenceOrder, `provider_round_${answerRound}_pending`,
        { provider_request: request, provider_response: initial.body },
        admission.provider_key, initial.provider_model, "pending", usage(initial.body),
        { duration_ms: initial.duration_ms, http_status: initial.status })
      await stageAsyncRound({ invocation_id: admission.invocation_id,
        gateway_call_id: initial.gateway_call_id, provider_response_id: responseId,
        input, request, route_key: route.route_key, answer_round: answerRound,
        provider_round: providerRound, evidence_order: evidenceOrder,
        provider_model: initial.provider_model,
        archive_receipt_id: begun.archiveReceiptId })
      return pendingResponse(admission.invocation_id)
    }
    const background = await waitForBackgroundResponse(initial, admission.invocation_deadline)
