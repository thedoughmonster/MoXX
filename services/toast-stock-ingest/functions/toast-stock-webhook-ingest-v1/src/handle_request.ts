...
export async function handleRequest(request: Request): Promise<Response> {
  ...
  const payload = parseToastStockPayload(rawBody)

  if (!payload) {
    return new Response("invalid payload", { status: 400 })
  }

  const secret = Deno.env.get("TOAST_STOCK_WEBHOOK_SECRET")
  ...
  try {
    ...
    console.error("Toast stock webhook persistence failed", error)
