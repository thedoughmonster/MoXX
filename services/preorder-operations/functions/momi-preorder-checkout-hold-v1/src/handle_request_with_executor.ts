...
import type { Failure, HoldExecutor } from "./types.ts";

const invalid: Failure = {
  code: "invalid_request",
  message: "The hold request is invalid.",
  retryable: false,
  next_action: "none",
};
  ...
  try {
    ...
    if (!execution.admitted) {
      const response = errorResponse(requestId, 429, {
        code: "rate_limited",
        message: "Too many checkout requests were received.",
        retryable: true,
