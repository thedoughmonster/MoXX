...
import { functionKey, type BootstrapInput,
  type BootstrapRead } from "./types.ts"

export async function readBootstrap(input: BootstrapInput): Promise<BootstrapRead> {
  ...
  const rows = await sql<BootstrapRead[]>`
    ...
    select admission.admitted,
      case when admission.admitted then momi_preorder.read_bootstrap_v1(
