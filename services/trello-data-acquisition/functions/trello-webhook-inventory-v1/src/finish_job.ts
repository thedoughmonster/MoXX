...
export async function finishJob(
  ...
  const rows = await database`
    ...
    from trello_acquisition.finish_webhook_inventory_v1(
      ${job.jobId}::uuid,
      ${job.capabilityToken},
      ${result.httpStatus},
  ...
  if (
    ...
    || (row.job_status !== "succeeded" && row.job_status !== "failed")
  ) throw new Error("Webhook inventory completion returned an invalid result")
