...
test("uses the durable operation and operation-bound marker", async () => {
  let deliveredMarker = ""
  const dependencies: DeliveryDependencies = {
    ...
    deliver(_operation, _key, _token, marker) {
      ...
      return Promise.resolve({
        httpStatus: 200,
        headers: {},
        payload: { id: "list-blocked" },
        rawText: '{"id":"list-blocked"}',
        errorCode: null,
      })
    },
...
test("does not deliver unavailable durable work", async () => {
  let deliveryCalls = 0
  const dependencies: DeliveryDependencies = {
