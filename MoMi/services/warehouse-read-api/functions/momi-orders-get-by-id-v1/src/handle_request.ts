import { parseOrderRead } from "./parse_request.ts"
...
export async function handleRequest(request: Request): Promise<Response> {
  ...
  const input = parseOrderRead(body)
  ...
  try {
    const row = await readOrder(input)
    ...
    if (!row.order_id || !row.order_document || !row.order_presentation ||
      !row.provenance || !row.freshness || !row.schema_version) {
      return Response.json({ ok: false, ...base,
        work_id: input.work_id, error: "order_not_found" }, { status: 404 })
    }
    return Response.json({ ok: true, ...base, work_id: input.work_id,
      order_id: row.order_id, schema_version: row.schema_version,
