...
const invalid: Failure = {
  code: "invalid_request", message: "The payment request is invalid.",
...
export async function handleRequestWithDependencies(
  ...
  try {
    ...
    if (!result) throw new Error("invalid_payment_result")
    if (result.error) return errorResponse(
      request, requestId, result.outcome === "conflict" ? 409 : 422,
      result.error,
    )
    if (!result.receipt) throw new Error("invalid_payment_result")
