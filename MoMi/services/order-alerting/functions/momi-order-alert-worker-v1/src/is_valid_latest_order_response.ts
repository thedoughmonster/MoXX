...
import { latestOrderContractKey } from "./types.ts"
import type { CanonicalReadCapability, ClaimedWork,
  LatestOrderApiSuccess } from "./types.ts"

const uuidPattern =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i

export function isValidLatestOrderResponse(
  body: unknown,
  job: ClaimedWork,
  readCapability: CanonicalReadCapability,
): body is LatestOrderApiSuccess {
  ...
  return job.api_contract_key === latestOrderContractKey &&
    readCapability.contract_key === latestOrderContractKey &&
    value.ok === true && value.contract_key === latestOrderContractKey &&
    value.contract_version === job.api_contract_version &&
    value.work_id === readCapability.work_id && value.order_id === job.order_id &&
