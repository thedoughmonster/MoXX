drop function momi_alerting.claim_order_alert_candidates(bigint, jsonb);
...
as $$
  with target_work as (
    ...
    select target.*, source.source_key,
      ...
      rule.alert_kind, route.destination_key,
    ...
    join momi_alerting.alert_routes as route
      ...
      and route.alert_kind = rule.alert_kind and route.is_enabled
...
comment on function
  ...
  'Claims configured alerts and snapshots one approved order presentation.';
revoke all on function
  momi_alerting.claim_order_alert_candidates(bigint, jsonb, jsonb)
