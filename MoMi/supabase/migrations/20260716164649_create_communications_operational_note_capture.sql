...
create function momi_communications.capture_operational_note_v1(p_note jsonb)
...
begin
  ...
  if exists (
    ...
    where p_note ? field_name
      and p_note -> field_name <> 'null'::jsonb
  ...
  if account_key is null or user_key is null or nullif(
    btrim(p_note ->> 'note_type'), '') is null
    or nullif(btrim(p_note ->> 'summary'), '') is null then
    raise exception 'Operational note account, user, type, and summary are required' using errcode = '22023';
  end if;
  ...
  if jsonb_typeof(questions) <> 'array' or jsonb_typeof(actors) <> 'array' then
    raise exception 'Operational note questions and actors must be arrays'
      using errcode = '22023';
  end if;
  ...
  if confidence_value is not null and (confidence_value < 0 or confidence_value > 1) then
    raise exception 'Operational note confidence must be between zero and one' using errcode = '22023';
  end if;
  ...
  from momi_communications.capture_openai_message_v1(
    ...
    'operational-note:' || capture_fingerprint, 'assistant',
    coalesce(nullif(p_note ->> 'occurred_at', '')::timestamptz, now()),
...
$$;
revoke all on function momi_communications.capture_operational_note_v1(jsonb) from public, anon, authenticated;
grant execute on function momi_communications.capture_operational_note_v1(jsonb) to service_role;
