create function toast_alerting.claim_hydrated_order_alert_candidates(
...
as $$
  with target_work as (
    ...
    where jsonb_typeof(input_order) = 'object'
      ...
      and input_order #> source.payload_path = source.expected_value
...
comment on function toast_alerting.claim_hydrated_order_alert_candidates(bigint, jsonb)
  is 'Claims configured alerts from one MoMi Order API response.';

revoke all on function
  toast_alerting.claim_hydrated_order_alert_candidates(bigint, jsonb)
