  ...
  command := $command$
    ...
    where target.job_id in (
      ...
      where eligible.operation_dispatch_rank
        ...
        and (not eligible.worker_batch_enabled
          or eligible.operation_dispatch_rank <= greatest(
            0, eligible.maximum_active_workers - active_worker_count
          ))
...
begin
  ...
  where jobname = 'momi-toast-acquisition-wakeup-v1';
  if dispatch_command not like '%active_worker_count%'
    or dispatch_command not like '%maximum_active_workers%'
    or dispatch_command not like '%limit 2 for update of job skip locked%'
  then raise exception 'Background worker capacity is invalid'; end if;
