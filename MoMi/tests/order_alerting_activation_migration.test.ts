...
test("cuts over order alerts without an ingestion race", async () => {
  ...
  assert.match(sql, /-- service-owner: order-alerting\s+begin;/)
  ...
  assert.match(sql, /minimum_recorded_at = case when active/)
...
test("stops only new legacy work and preserves terminal records", async () => {
  ...
  assert.match(sql, /case when active[\s\S]*minimum_recorded_at/)
